...
def getopts(argv):
	...
	while argv:
		try:
			...
			if argv[0] == '-h':
				print(bcolors.INFO+"[*] "+bcolors.RESET+"usage: ./kalidomains.py [-h] -t target [-o output file] [-a]")
				help()
				sys.exit(0)
		argv = argv[1:] 
	return opts
...
def main():
	...
	if len(sys.argv) < 2:
		...
		print(bcolors.INFO+"[*] "+bcolors.RESET+"usage: ./kalidomains.py [-h] -t target.com [-o output file] [-a]")
		sys.exit(0)
